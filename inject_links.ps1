...
Set-Content -Path "$base_dir\app.js" -Value $appJsContent -Encoding UTF8

Get-ChildItem -Path $base_dir -Directory | ForEach-Object {
    $codePath = Join-Path $_.FullName "code.html"
    if (Test-Path $codePath) {
        $content = Get-Content -Path $codePath -Raw
        if ($content -notmatch '<script src="../app.js"></script>') {
            $content = $content -replace "</body>", "<script src=`"../app.js`"></script>`n</body>"
            Set-Content -Path $codePath -Value $content -Encoding UTF8
        }
    }
}
